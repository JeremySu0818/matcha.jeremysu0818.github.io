...
function isNearBowl(position: Tuple3) {
  return Math.hypot(position[0] - 0.49, position[2]) <= BOWL_DROP_RADIUS;
}
...
export function MatchaScene({
  ...
  useFrame(({ clock }) => {
    ...
    if (mode === "manual") {
      const topCamera = new Vector3(0.49, mobile ? 9.2 : 8.2, 0.02);
      camera.up.set(1, 0, 0);
      camera.position.lerp(topCamera, 0.12);
      (camera as PerspectiveCamera).lookAt(0.49, -0.5, 0);
    } else {
      camera.up.set(0, 1, 0);
    ...
    if (bowlRef.current) {
      ...
      bowlRef.current.position.x = mix(0.49, 0.49, finale);
  ...
  return (
    <>
      ...
      <Float speed={0.75} rotationIntensity={0.08} floatIntensity={0.08}>
        <group ref={bowlRef} position={[0.49, 0, 0]}>
          <Model
